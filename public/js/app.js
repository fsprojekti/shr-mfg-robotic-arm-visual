...
const x_submit = document.getElementById("x-button")
const x = document.getElementById("x-input")
const y_submit = document.getElementById("y-button")
const y = document.getElementById("y-input")
const z_submit = document.getElementById("z-button")
const z = document.getElementById("z-input")
const image_processing = document.getElementById("img_proces")
const image = document.getElementById("image")
const start_button = document.getElementById("start_auto")
const stop_button = document.getElementById("stop_auto")
const offerId = document.getElementById("offerId")
const dispatch_button = document.getElementById("dispatch_w_t")
...
image_processing.addEventListener("click", () => {
    image_processing.setAttribute("disabled", "disabled");
    socket.emit("img_proces", (error) => {
        if(error) {
            return console.log(error);
        }
        console.log("message delivered");
...
socket.on("addOffer", async(message) => {
    let data = message;
    console.log(data.id);
    await delay(500);
    await addOffer(data.id,data.location,data.package);
})
socket.on("removeOffer", async(message) => {
    let data = message;
    console.log(data.id);
    await delay(500);
    await removeOffer(data.id);
})
socket.on("editOffer", async(message) => {
    let data = message;
    console.log(data.id);
    await delay(500);
...
dispatch_button.addEventListener("click", () => {
    const message = offerId.value;
    dispatch_button.setAttribute("disabled", "disabled");
    if(message === "") {
        dispatch_button.removeAttribute("disabled");
        throw "please set value";
    }
    socket.emit("dispatch_w_t", message,(error) => {
        dispatch_button.removeAttribute("disabled");
        offerId.value = "";
        if(error) {
            return console.log(error);
...
store_button.addEventListener("click", () => {
    const message = offerId.value;
    store_button.setAttribute("disabled", "disabled");
    if(message === "") {
        store_button.removeAttribute("disabled");
        throw "please set value";
    }
...
let rendered = Mustache.render(template, { message: 'Luke' });
